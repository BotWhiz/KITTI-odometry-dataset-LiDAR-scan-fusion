...
std::string temp_dir;
size_t max_points_per_file = 500000; // 每个文件保存的最大点数
...
Eigen::Matrix3d R_Tr1;
Eigen::Vector3d t_Tr1;

Eigen::Matrix4d Tr;
Eigen::Matrix4d Tr_inv;

Eigen::Matrix4d Tr1;
Eigen::Matrix4d Tr1_inv;
...
void setupTransformation() {
    ...
    R_Tr1 << 0, -1, 0, 0, 0, -1, 1, 0, 0;
    
    t_Tr1 << 0, 0, 0;
    
    Tr1 << R_Tr1(0,0), R_Tr1(0,1), R_Tr1(0,2), t_Tr1(0),
          R_Tr1(1,0), R_Tr1(1,1), R_Tr1(1,2), t_Tr1(1),
          R_Tr1(2,0), R_Tr1(2,1), R_Tr1(2,2), t_Tr1(2),
          0, 0, 0, 1;
}
...
void publishTFAndMarker(const Pose& pose, const ros::Time& timestamp, int index) {
    ...
    tf_broadcaster->sendTransform(tf::StampedTransform(transform, timestamp, "map", "camera_init"));
    ...
    marker.pose.position.z = pose.t.z();
    marker.pose.orientation.w = 1.0;
    marker.scale.z = 0.5; // 字体大小
    marker.color.a = 1.0; // 透明度
    marker.color.r = 1.0;
    marker.color.g = 0.0;
    marker.color.b = 0.0;
    marker.text = std::to_string(index); // 显示的文字内容
...
void callback(const nav_msgs::Odometry::ConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    // 发布TF和Marker
    publishTFAndMarker(pose, odom_msg->header.stamp, pose_vec.size() - 1);
...
int main(int argc, char** argv) {
    ...
    marker_pub = nh.advertise<visualization_msgs::MarkerArray>("pose_markers", 1);

    // 捕获SIGINT信号（如Ctrl+C），保存点云地图
