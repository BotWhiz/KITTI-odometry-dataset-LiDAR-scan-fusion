...
std::string temp_dir;
size_t max_points_per_file = 10000000; // 每个文件保存的最大点数
...
void publishTFAndMarker(const Pose& pose, const ros::Time& timestamp, int index) {
    ...
    ros::Time updated_timestamp = timestamp + ros::Duration(index * 0.0001); // 确保每个TF变换具有唯一的时间戳
